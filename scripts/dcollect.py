...
from data_collection.data_collection_script import get_argument_parser as get_main_arg_parser
...
if __name__ == "__main__":
    ...
    with open(killfilename, "w") as killfile:
        for it, addr in enumerate(all_addresses):
            ...
            os.system(f"rsync -rz {code_src} {code_tget} --exclude '__pycache__' --exclude '{args.output_dir}' --exclude 'data_collection/data'")

            job_id = id_generator()

            command = " ".join(
                ["python data_collection/data_collection_script.py"]
                + raw_args
                + [
                    "--extra_tag",
            ...
            tmux_name = f"data_collection_{time_str}_{global_job_id}_{job_id}_machine{it}"
