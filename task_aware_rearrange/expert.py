...
class SubtaskAndActionExpertSensor(AbstractExpertActionSensor):
    ...
    def update(
        ...
        if isinstance(task, WalkthroughTask):
            ...
            if self.verbose:
                get_logger().info(
                    f"\t\tSeen objects during exploration ({n_obj_seen} objects / {task.total_pickupable_or_openable_objects} objects ({prop_obj_seen:.2f}%)): \n"
                    f"\t\t\tPickupables: {task.seen_pickupable_objects}\n"
                    f"\t\t\tOpenables: {task.seen_openable_objects}\n"
                )
                get_logger().info(
                    f"\t\tCovered areas: \n"
                    f"\t\t\tXZ ({len(task.visited_positions_xz)} / {len(task.reachable_positions)}({prop_visited_xz:.2f}%))\n"
                    f"\t\t\tXZR ({len(task.visited_positions_xzrsh)} / {4 * len(task.reachable_positions)}({prop_visited_xzr}%))\n"
                )
    ...
    def _update_expert_subtask(
        ...
        if isinstance(task, WalkthroughTask):
            if not replan_subtask:
                remaining_objects = {
                    obj
                    for obj in task.pickupable_or_openable_objects()
                    if obj not in task.seen_pickupable_objects and obj not in task.seen_pickupable_objects
                }
