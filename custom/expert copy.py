...
class SubtaskOnePhaseExpertSensor(AbstractExpertActionSensor):

    def __init__(
        self, 
        action_space: Optional[Union[gym.Space, int]] = None, 
        uuid: str = "subtask_expert", 
        ...
        **kwargs: Any
        ...
        super().__init__(action_space, uuid, expert_args, nactions, use_dict_as_groups, **kwargs)
        self.max_priority_per_object = max_priority_per_object
        self.object_name_to_priority: defaultdict = defaultdict(lambda: 0)

        self.expert_subtask_list: List[Optional[Tuple[int, int, int]]] = []
    ...
    def get_object_names_from_current_view(
        ...
        with include_object_data(env.controller):
            ...
            # get_logger().info(f'FOUND Objects to rearrange: {visible_rearrange_objects}')
    ...
    def update(
        ...
        if action_taken is not None:
            ...
            if "Explore" in last_expert_subtask:
                ...
                arg_type = next(
                    (
                        o['objectType'] for o in env.last_event.metadata['objects']
                ...
                if action_str == target_action and action_success:
                    if "PickupObject" not in last_expert_subtask:
                        self.object_name_to_priority[
                            self._last_to_interact_object_pose["name"]
                        ] += 1
                    self._last_subtask = None
        ...
        self._generate_and_record_expert_subtask(task)

    def _generate_and_record_expert_subtask(self, task: UnshuffleTask):
        ...
        expert_subtask_dict = self._generate_expert_subtask_dict(task)
        type_id = SUBTASK_TYPES_TO_IDX[expert_subtask_dict["subtask"]]
        arg_id = (
            OBJECT_TYPES_TO_IDX[expert_subtask_dict["arg"]["objectId"].split("|")[0]] 
            if expert_subtask_dict["arg"] is not None 
            else -1
        )
        target_map_id = (
            MAP_TYPES_TO_IDX[expert_subtask_dict["arg"]["target_map"]]
            if (
                expert_subtask_dict["arg"] is not None 
                and "target_map" in expert_subtask_dict["arg"]
            )
            else 0
        )
    ...
    def _generate_expert_subtask_dict(self, task:UnshuffleTask):
        ...
        if held_object is not None:
            ...
            if held_object["name"] in self.seen_obj_names_walkthrough:
            ...
            if type_id == 2:
                ...
                if (
                    ...
                    self._last_subtask = "OpenObject"
                    self._last_to_interact_object_pose = goal_obj_pos
                    self._last_to_interact_object_pose["target_map"] = "Walkthrough"
        ...
        return dict(subtask=self._last_subtask, arg=self._last_to_interact_object_pose)
    ...
    def query_expert(
        ...
        self.update(task=task, action_taken=last_action_ind, action_success=last_action_success)

        return self.expert_subtask(task), True


class LowLevelOnePhaseExpertSensor(AbstractExpertActionSensor):

    def __init__(
        self, 
        action_space: Optional[Union[gym.Space, int]] = None, 
        uuid: str = "low_level_expert", 
        expert_args: Optional[Dict[str, Any]] = None, 
        nactions: Optional[int] = None, 
        use_dict_as_groups: bool = True, 
        max_priority_per_object: int = 3,
        ...
        **kwargs: Any,
        ...
        self._last_subtask: Optional[str] = None
        self._last_subtask_argument: Optional[Dict[str, Any]] = None
    ...
    def update(
        ...
        if task.unshuffle_env.held_object is not None:
            self._last_held_object_name = held_object["name"]
    ...
    def _generate_and_record_expert_subtask_and_action(self, task: UnshuffleTask):
        ...
        get_logger().info(
            f'    Expert Action: {action_str}[{task.action_names().index(action_str)}]'
        )
        try:
    ...
    def _generate_expert_subtask_dict(self, task: UnshuffleTask):
        ...
        get_logger().info(
            f'    During _generate_expert_subtask_dict()'
        )
        get_logger().info(
            f'        Guess what to do for next subtask...'
        )
        ...
        if held_object is not None:
            ...
            if held_object["name"] in self.seen_obj_names_walkthrough:
                ...
                get_logger().info(
                    f'        object {held_object["name"]} is not found in walkthrough env...'
                )
                self._last_subtask = "Explore"
            self._last_to_interact_object_pose["target_map"] = "Walkthrough"      
    ...
    def _generate_expert_action_dict(self, task: UnshuffleTask):
        ...
        get_logger().info(
            f'    During _generate_expert_action_dict()'
        )
        get_logger().info(
            f'        expert_nav_action: {expert_nav_action} | obj: {self._last_to_interact_object_pose["name"]}'
        )
        if expert_nav_action is None:
            ...
            if self._last_to_interact_object_pose["name"] not in visible_objects:
                get_logger().info(
                    f'        Could not find the target obj in current view.'
                )
    ...
    def query_expert(
        ...
        self.update(
            task=task, 
            shortest_path_navigator=shortest_path_navigator, 
            action_taken=last_action_ind, 
            action_success=last_action_success
        )

        expert_subtask = IDX_TO_SUBTASK_TYPE[int(self.expert_subtask(task=task) / (NUM_OBJECT_TYPES * len(MAP_TYPES)))]
        expert_action = task.action_names()[self.expert_action(task)]

        return self.expert_action(task), True
    ...
    def get_observation(
        ...
        **kwargs: Any
        ...
        return super().get_observation(env, task, *args, **kwargs)
...
class OnePhaseSubtaskAndActionExpertSensor(AbstractExpertActionSensor):
    ...
    def query_expert(
        ...
        assert expert_sensor_group_name is not None
        env = task.unshuffle_env
        if not hasattr(env, "shortest_path_navigator"):
            assert task.num_steps_taken() == 0
            env.shortest_path_navigator = ShortestPathNavigatorTHOR(
