...
def render_geometries(geometry, animate=False, num_frames=18, centroid=None):
    ...
    vis.create_window(width=int(intrinsics["width"]/2), height=int(intrinsics["height"]/2), visible=False)
    time.sleep(0.1)
    ...
    # viewportcam.intrinsic.width = intrinsics["width"]
    # viewportcam.intrinsic.height = intrinsics["height"]
    if T_extrinsic is not None:
        viewportcam.extrinsic = T_extrinsic
    if "intrinsic_matrix" in intrinsics:
        viewportcam.intrinsic.intrinsic_matrix = intrinsics["intrinsic_matrix"]/2
