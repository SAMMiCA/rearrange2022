...
from custom.hlsm.utils.projection_utils import project_3d_camera_points_to_2d_pixels, project_3d_points
...
class Voxel3DObservability(nn.Module):
    ...
    def forward(
        ...
        dtype = torch.float32 if str(voxel_grid.data.device) == "cpu" else torch.half
        ...
        voxel_observability_mask = torch.max(
            ...
            (
                voxel_grid.data.max(1, keepdim=True).values > 0.2
            ).long()
        )
        voxel_observability_mask = voxel_observability_mask.long()

        # output
        observability_grid = VoxelGrid(
            voxel_observability_mask.type(dtype), # Observability
